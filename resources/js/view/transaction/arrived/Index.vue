                                ...
                                <app-data-table :table-config="tableConfig" @get-data="getDataTable">
                                    <template v-slot:body>
                                        <tr v-for="(context, index) in tableConfig?.data?.body">
                                            ...
                                            <td valign="middle">
                                                <span :class="`badge badge-light-primary`" v-if="context?.status_id == 10"><b>{{ context?.status?.name }}</b></span>
                                                <span :class="`badge badge-light-danger`" v-if="context?.status_id == 11"><b>{{ context?.status?.name }}</b></span>
                                                <span :class="`badge badge-light-success`" v-if="context?.status_id == 12"><b>{{ context?.status?.name }}</b></span>
                                            </td>
    ...
    export default {
        ...
        methods: {
            ...
            getDataTable(){
                ...
                this.$axios().get(`transaction/data-table/5`, {params: this.tableConfig?.config})
                    .then(res => {
